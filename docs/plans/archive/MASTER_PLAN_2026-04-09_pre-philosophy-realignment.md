...
Reason: Snapshot of MASTER_PLAN.md taken alongside the NORTH_STAR
philosophy-realignment archive. MASTER_PLAN was rewritten the same day
as NORTH_STAR and inherited the same blind spots. This snapshot
preserves the version that pointed at the now-archived NORTH_STAR so
the realignment can be traced cleanly.

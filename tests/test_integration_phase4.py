"""Integration tests — APEX reflect adapter and ApexConfig roundtrip."""
...
class TestReflectAdapterDirections:
    ...
    def test_high_fdr_suggests_radar_threshold(self):
        metrics = ReflectMetrics(
            ...
            net_pnl=100.0, gross_pnl=200.0, total_fees=50.0,
        )
    ...
    def test_low_win_rate_suggests_pulse_confidence(self):
        metrics = ReflectMetrics(
            total_round_trips=10, win_rate=35.0, fdr=10.0,
            net_pnl=50.0, gross_pnl=100.0, total_fees=5.0,
        )
        directions = suggest_research_directions(metrics)
    ...
    def test_both_issues_present(self):
        metrics = ReflectMetrics(
            total_round_trips=10, fdr=35.0, win_rate=35.0,
            net_pnl=50.0, gross_pnl=100.0, total_fees=30.0,
        )
        directions = suggest_research_directions(metrics)
    ...
    def test_healthy_metrics_suggest_relaxing(self):
        metrics = ReflectMetrics(
            total_round_trips=10, win_rate=60.0, fdr=10.0,
            net_pnl=500.0, gross_pnl=600.0, total_fees=20.0,
        )
        directions = suggest_research_directions(metrics)
...
class TestApexConfigRoundtrip:
    ...
    def test_json_roundtrip_non_defaults(self, tmp_path):
        original = ApexConfig(
            total_budget=25_000.0, max_slots=5, leverage=15.0,
            radar_score_threshold=220, pulse_confidence_threshold=85.0,
            daily_loss_limit=750.0, max_same_direction=1,
            min_hold_ms=3_000_000, slot_cooldown_ms=600_000,
            cooldown_duration_ms=2_400_000, entry_order_type="Gtc",
        )
    ...
    def test_phase3_fields_survive_roundtrip(self, tmp_path):
        original = ApexConfig(
            min_hold_ms=5_000_000, slot_cooldown_ms=900_000,
            cooldown_duration_ms=3_600_000, entry_order_type="Alo",
        )

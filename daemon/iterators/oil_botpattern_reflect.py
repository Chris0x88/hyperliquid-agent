...
class OilBotPatternReflectIterator:
    ...
    def _load_closed_trades(self) -> list[dict]:
        """Load closed oil_botpattern trades from the main journal.

        When Sub-5 is in decisions_only=true (shadow mode), also appends rows
        from the shadow-trades JSONL so L2 has learning signal. Each shadow row
    ...
    def _load_shadow_trades(self) -> list[dict]:
        ...
        fields to canonical closed-trade fields so compute_weekly_proposals sees
        a uniform schema. Each row carries source="shadow" for attribution.
        """
        ...
        try:
            ...
            log.warning("oil_botpattern_reflect: failed to read shadow trades %s: %s",
                        shadow_path, e)
        return out
